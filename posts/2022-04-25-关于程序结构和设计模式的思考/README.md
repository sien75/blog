...
当然, 如果这个状态是一个引用类型的变量, 那么外部拿到这个引用类型变量时也是可以修改其内部数据的, 这就是数据可变性带来的弊端.

这时get接口可以返回一个状态的深拷贝来解决上面的问题. 或者, 使用`Object.freeze`方法, 在类属性更新时就将其冻结.

个人认为状态的更新可以和`immer.js`配合, 一方面冻结了属性, 另一方面可以方便地实现共享内存(见[享元模式](#享元模式)).

典型的接口类型: "增", "删", "改", "查".

如果这个类的某个方法仅仅是处理输入数据然后返回, 没有任何其他依赖(即"纯函数"), 那这就不需要类了, 因为和函数相比, 类的区别就是保存了一套状态. 这时, 应该考虑把这个方法作为单独的函数.
...
首先这并不代表设计模式没有意义, 设计模式提出时间较早, 其实是对当时编程语言的能力的补齐. 现在的各流行编程语言已经或多或少在语法层面上支持了一些设计模式理念, 所以这可能是导致很多设计模式在现在看起来比较繁琐无用的原因.
...
观察者模式, 也叫发布订阅模式, 可能是我最早接触到的设计模式了.

Vue中的监听数据变动并更新视图的思路, 和观察者模式就是相同的. Web API中的事件处理器, 如`onclick`, 其实和观察者模式的思想也是类似的, 但是其并非由应用开发者的代码触发, 而是用户行为触发.
